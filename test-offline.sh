...
for s in 0 1; do for m in 0 1 2; do
	...
	checkfail "wrong name" 0 "$s" "$m" "$t" "$ROOT" "$CHAIN" \
	    "whatever"
	checkfail "null CA" 0 "$s" "$m" "$t" "" "$CHAIN" "$HOST"
	...
	checkpass "valid TA" 2 "$s" "$m" "$tlsa" "" "$CHAIN" \
	    "$HOST"
	checkpass "valid TA+CA" 2 "$s" "$m" "$tlsa" "$ROOT" "$CHAIN" \
	    "$HOST"
	checkpass "sub-domain match" 2 "$s" "$m" "$tlsa" "" "$CHAIN" \
	    whatever ".$DOMAIN"
	checkfail "wrong name" 2 "$s" "$m" "$tlsa" "" "$CHAIN" \
	    "whatever"
	checkfail "non-TA" 2 "$s" "$m" "0.$HOST" "" "$CHAIN" \
	    "$HOST"
	checkpass "depth 0 TA" 2 "$s" "$m" "$ss" "" "$ss" "${HOST}" 
	checkfail "depth 0 TA namecheck" 2 "$s" "$m" "$ss" "" "$ss" \
	    whatever
    done
    ...
    checkfail "wrong name" 1 "$s" "$m" "0.$HOST" "$ROOT" "$CHAIN" \
    	whatever
    checkfail "null CA" 1 "$s" "$m" "0.$HOST" "" "$CHAIN" "$HOST"
    ...
    checkpass "valid EE" 3 "$s" "$m" "0.$HOST" "" "$CHAIN" whatever
    checkfail "wrong EE" 3 "$s" "$m" "1.$HOST" "" "$CHAIN" whatever
